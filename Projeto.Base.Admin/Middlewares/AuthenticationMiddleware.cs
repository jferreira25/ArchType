...
namespace Projeto.Base.Admin.Middlewares
{
    public class AuthenticationMiddleware
    {
        ...
        {
            if (context.Request.Path.Value.ToLower().EndsWith("login") || context.Request.Path.Value.ToLower().EndsWith("health") || context.Request.Path.Value.ToLower().Contains("healthchecks") || context.GetEndpoint().Metadata?.GetMetadata<IAllowAnonymous>() is object)
            {
